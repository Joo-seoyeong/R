...
# 기대비율(고졸 vs 합격 = 0.363)
# 1) 기댓값 = (셀의 행 합 * 셀의 열 합) / 전체 합
p_value = (89*135)/225
p_value # 53.4
# 2) 기대비율 = (관측값-기댓값)^2 / 기댓값
p_rate = (49-p_value)^2/p_valued
p_rate # 0.3625468

############ 2. 카이제곱 검정 ###############
# 귀무가설: 부모의 학력 수준이 자녀의 대학진학에 영향을 미치는가?(두 변수는 관련이 없다)
CrossTable(x=df$Level,y=df$Pass, chisq=TRUE)
# Chi^2 =  2.766951     d.f. =  2     p =  0.2507057
# 검정통계량 ; Chi^2=2.766951 / 자유도:2
chi_2 = 0.544+0.363+1.026+0.684+0.091+0.060
chi_2 # 2.768

# 자유도(DF) : 샘플의 수(n)에서 자유롭게 선택할 수 있는 수 
# n-1
df = (3-1)*(2-1)
df # 2
# 유의확률 : p=0.2507057 > 유의수준(0.05) -> 귀무가설 채택 
...
# <검정통계량 해석>
# X-squared = 14.2 , df=5
# X-squared = 14.2 > 11.071 (카이제곱분포표) -> 귀무가설 기각
...
head(data)
table(data$method) # 집단 변수
table(data$survey) # 등간 척도
# 2. 변수 리코딩 - 코딩 변경
...
# p_value가 알파보다 크므로 귀무가설을 기각할 수 없다. 
# 즉 교육방법(집단)에 따른 만족도에 차이가 없다고 할 수 있다. 
...
chisq.test(x,y)
# X-squared = 156.27, df = 68, p-value = 6.666e-09(6.6*10^-9) 
# [해설] 꽃의 종별로 꽃받침의 길이에 차이가 있다고 할 수 있다. 

table(x) # setosa versicolor  virginica
